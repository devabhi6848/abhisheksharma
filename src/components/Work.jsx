  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-[#0e0e0e] border border-white/8 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-10"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between mb-10 gap-4">
          <div>
            <p className="font-mono text-xs text-accent tracking-widest mb-3">{project.index}</p>
        ...
        {[
          ...
          <div key={label} className="mb-8">
            <p className="font-mono text-xs text-white/30 tracking-widest uppercase mb-3">{label}</p>
      ...
      <div className="space-y-0">
        {projects.map((p, i) => (
          <motion.div
            ...
            className="group border-t border-white/8 py-10 grid grid-cols-1 md:grid-cols-[80px_1fr_auto] gap-6 items-start cursor-pointer hover:border-white/20 transition-colors duration-300"
            onClick={() => setActive(p)}
          >
            <span className="font-mono text-xs text-white/20 pt-1">{p.index}</span>
            <div>
            ...
            <div className="text-right">
              <p className="font-mono text-xs text-white/25 mb-4">{p.role}</p>
